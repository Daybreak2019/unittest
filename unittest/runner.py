...
class TextTestResult(result.TestResult):
    ...
    def isExcCale (self, test):
        # wen, for execute single case
        cur_case =  os.environ.get ("case_name")
        if cur_case is None:
            return True
        else:
            cur_case = cur_case.replace ("#", " ")
            if cur_case == str (test):
                return True
            else:
                return False

    def startTest(self, test):
        if self.testDump (test) == True:
            return
        if self.isExcCale (test) == False:
            return

        os.environ["case_step"] = "setup"
        print ("@@@@ startTest -> ", str (test))
